                ...
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    if event.button == 1:  # 左键点击
                        if self.board[y][x] == -1:
                            ...
                            self.dict[(x, y)] = 1  # 记录已经打开的格子
                    elif event.button == 3:  # 右键点击
                        if (x, y) in self.dict and self.dict[(x, y)] != 1:
                            self.dict[(x, y)] = 2  # 记录已经标记的格子
                            self.revealed[y][x] = not self.revealed[y][x]  # 翻转格子
                        elif (x, y) in self.dict and self.dict[(x, y)] == 1:
                            pass
